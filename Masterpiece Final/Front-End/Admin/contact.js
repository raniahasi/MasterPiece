async function getAllContact() {
  var requist = await fetch(
    "https://localhost:7159/api/contactUs/getAllContact"
  );
            ...
            <tr>
                ...
                <td>
                    ...
                    <a class="btn-icon btn btn-ghost btn-sm rounded-circle" href="#" role="button"
                        ...
                        aria-expanded="false"
                        onclick="contactId(${element.contactId},${
      element.status
    })"
                        >
...
async function getContactByStatus() {
  var requist = await fetch(
    "https://localhost:7159/api/contactUs/getContactByStatus"
  );
  var response = await requist.json();
  ...
  response.forEach((element) => {
    getAll.innerHTML += `
            <tr>
                ...
                <td>
                    <span class="badge ${
                      element.status == 0 ? "bg-info" : "bg-success"
                    } badge-dot me-1"></span>${
      element.status == 0 ? "Unanswered" : "Answered"
    }
                </td>
